...
def gdf_to_sql_old(table_name, gdf, num_rows, random_var, rand_var_types, rand_var_names, extra_var, extra_var_types, extra_var_name, extra_var_dict, directory):
...
def generate_vars(gdf, rand_var_types, rand_var_names, rand_var_params, null):
    for count, type in enumerate(rand_var_types):
        if type == 'int':
            ...
            gdf[f'{rand_var_names[count]}'] = [exrex.getone(rand_var_params[count]) for i in range(len(gdf.index))]
        else:
            return
    return gdf
...
def radian():
    ...
    if random_var:
        gdf_out = generate_vars(gdf_out, rand_var_types, rand_var_names, rand_var_params, null)
